...
Application::Application(int window_width, int window_height, SDL_Window* window)
{
...

	{
		...
		//volu->loadPVM("data/volumes/Daisy.pvm");
		volu->loadPVM("data/volumes/CT-Abdomen.pvm");
		//volu->loadPNG("data/volumes/volumes/teapot_16_16.png", 16.0, 16.0);

		Texture* volu_text = new Texture();
		volu_text->create3DFromVolume(volu, GL_CLAMP_TO_EDGE);
		
		SceneNode* node = new SceneNode("Volume node");
		node->mesh = new Mesh();
		node->mesh->createCube();

		VolumeMaterial* mat = new VolumeMaterial();
		mat->texture = volu_text;

		mat->blueNoise = Texture::Get("data/blueNoise.PNG");
		mat->LUT = Texture::Get("data/lut1.PNG");

		mat->c = vec4(1.0, 1.0, 1.0, -5.0);
		node->material = mat;

		

		//mat->LUTbonsai = Texture::Get("data/lut6.PNG");
		//setscale
		//node->model.setScale(1.0, volu_text->height/volu_text->width, volu_text->depth /volu_text->width);
		node->model.setScale(1, 
			(volu->height*volu->heightSpacing)/(volu->width*volu->widthSpacing),
			(volu->depth* volu->depthSpacing) / (volu->width * volu->widthSpacing));
